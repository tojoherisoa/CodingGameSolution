...
Poly mul(const Poly& a, const Poly& b) {
    ...
    // multiply by bytes using the lookup table
    uint8_t* pa = (uint8_t*)a.data;
    uint8_t* pb = (uint8_t*)b.data;
    uint16_t* pr = (uint16_t*)res.data; // this is safe-ish because we accumulate into res using XOR
    ...
    for (int i = 0; i < bytes_a; ++i) {
        ...
        for (int j = 0; j < bytes_b; ++j) {
            ...
            // accumulate p into res offset by (i+j) bytes
            // Since res is 32-bit aligned we do it safely byte by byte
            int offset = i + j;
...
void divmod(const Poly& a, const Poly& b, Poly& q, Poly& r) {
    ...
    if (b.is_zero()) return;
    int db = b.deg;
    uint32_t b_msb_inv = 1; // GF(2) so it's always 1
    
    while (r.deg >= db) {
        int shift = r.deg - db;
        q.data[shift / 32] ^= (1U << (shift % 32));
        
        // inline shift and xor for b into r
        int word_shift = shift / 32;
        int bit_shift = shift % 32;
        if (bit_shift == 0) {
...
Poly square_mod(const Poly& a, const Poly& m) {
    // Squaring in GF(2) can be done by inserting 0 between bits
    Poly sq;
...
int main() {
    ...
    int S = 256;
    string in_str[] = {
        "320a18d5", "b61b13f6", "1aaaa61c", "0afea209", 
        "6cc9659d", "dc503569", "ef4d0ef5", "73b746c5", 
        "b8fb36d3", "7616e9d6", "b21251c4", "00000000", 
    ...
    Poly B;
    for (int i = 0; i < N1; ++i) {
        B.data[i] = stoul(in_str[i], nullptr, 16);
    }
    B.update_deg();
    ...
    cout << "Time ms: " << (double)(end - start)/CLOCKS_PER_SEC * 1000 << endl;
